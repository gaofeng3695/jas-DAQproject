...
function getPrivilege() {
	...
	$.getJSON(rootPath + "/jasframework/privilege/privilege/getPageConfig.do", {
		"menuFunctionNumber" : menuid,
		"appNumber" : 1,
		"r" : new Date().getTime()
	}, function(data) {
		$.each(data, function(i, item) {
			$.each(item.children, function(i, item1) {
				$("#" + item1.attributes.privilegeNumber).show();
				allPriveilegeNumber.push(item1.attributes.privilegeNumber);
			});
		});
...
function getDataPrivilege(){
	...
	$.getJSON(rootPath + "/jasframework/privilege/privilege/getPageConfig.do", {
		...
		$.each(data, function(i, item) {
			$.each(item.children, function(i, item1) {
				$(".a" + item1.attributes.privilegeNumber).show();
			});
		});
	});
}
