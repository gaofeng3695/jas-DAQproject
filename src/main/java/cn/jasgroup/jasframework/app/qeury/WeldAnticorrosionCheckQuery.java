...
public class WeldAnticorrosionCheckQuery extends BaseJavaQuery {
	...
	public String getQuerySql() {
		String sql = "SELECT wac.oid, wac.project_oid, pro.project_name, wac.pipeline_oid, pi.pipeline_name, wac.tenders_oid, te.tenders_name, "
					...
					+ "to_char(wac.buckle_date, 'YYYY-MM-DD') as buckle_date, wac.buckle_anticorrosive_type, "
					+ "d.code_name as buckle_anticorrosive_type_name, wac.buckle_anticorrosive_grade, dm.code_name as buckle_anticorrosive_grade_name, "
					+ "wac.buckle_material_batch_num, wac.derusting_grade, dom.code_name as derusting_grade_name, wac.pipe_mouth_clean, "
					+ "wac.sandblasting_and_derusting, wac.pipe_mouth_preheat, wac.epoxy_primer, wac.baking_check, wac.overlap_check, "
					+ "wac.appearance_check, wac.electric_spark_leak_detection, wac.buckle_conclusion,"
					+ "case when wac.buckle_conclusion=1 then '合格' when wac.buckle_conclusion=0 then '不合格' end as buckle_conclusion_name, "
					+ "wac.anticorrosion, wac.construct_unit, u.unit_name as construct_unit_name, wac.supervision_unit, "
					+ "pu.unit_name as supervision_unit_name, wac.supervision_engineer, wac.collection_person, to_char(wac.collection_date, 'YYYY-MM-DD') as collection_date,"
					...
					+ "wac.remarks, wac.create_user_id, wac.create_user_name, wac.create_datetime, wac.modify_user_id, wac.modify_user_name, wac.modify_datetime, "
					...
					+ "case when wac.epoxy_primer=1 then '合格' when wac.epoxy_primer=0 then '不合格' end as epoxy_primer_name,case when wac.baking_check=1 then '合格' when wac.baking_check=0 then '不合格' end as baking_check_name,"
					...
					+ "LEFT JOIN (select oid, weld_code, active from daq_construction_weld where active=1) cw ON cw.oid = wac.weld_oid "
