...
public class QualityStatsDao {
	 ...
	 * <p>功能描述：根据项目查所有的施工单位和建设单位。</p>
	  ...
	  * @return
	  * @since JDK1.8。
	  * <p>创建日期:2018年12月18日 下午3:35:29。</p>
	  * <p>更新日期:[日期YYYY-MM-DD][更改人姓名][变更描述]。</p>
	 */
	public List<Map<String, Object>> getAllProjectAndConstructUnitByProjectOid(String projectOid) {
		String sql = "select DISTINCT t.oid as key,t.unit_name as value,t.hierarchy from pri_unit t "
					...
					+ "and (t.hierarchy like 'Unit.0001.0001%' or t.hierarchy like 'Unit.0001.0005%') "
					+ "and i.project_oid=:projectOid ORDER BY t.hierarchy;";
		List<Map<String, Object>> list = commonDataJdbcDao.queryForList(sql, ImmutableMap.of("projectOid", projectOid));
		return list;
	}
