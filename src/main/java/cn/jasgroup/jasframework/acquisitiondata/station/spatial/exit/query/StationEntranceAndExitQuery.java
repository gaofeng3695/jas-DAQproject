package cn.jasgroup.jasframework.acquisitiondata.station.spatial.exit.query;
...
public class StationEntranceAndExitQuery extends BaseJavaQuery {
	...
	public String getQuerySql() {
		String sql = "SELECT sta.oid,sta.device_code,sta.device_name,sta.device_type,case when sta.device_type = 0 then '进站' when sta.device_type = 1 then '出站' when sta.device_type = 2 then '分输' else '' end as device_type_name,"
					+ "	sta.weld_oid,spw.weld_code,sta.connection_pipeline_type,sta.connection_pipeline_code,sta.median_stake_oid,me.median_stake_code,"
					...
					+ "sta.pipeline_oid,pi.pipeline_name,sta.pipe_station_oid,ps.pipe_station_name,u.unit_name as construct_unit_name,pu.unit_name as supervision_unit_name,"
					+ "to_char(sta.collection_date, 'YYYY-MM-DD') as collection_date,case when sta.approve_status = -1 then '驳回' when sta.approve_status = 1 then '待审核' when sta.approve_status = 2 then '审核通过' "
					+ "else '未上报' end as approve_status_name FROM daq_station_entrance_and_exit sta "
					...
					+ "LEFT JOIN (select oid,median_stake_code from daq_median_stake where active=1) me ON me.oid = sta.median_stake_oid "
